...
extension BlogPost {
    ...
    func toViewPostWithoutTags(
        authorName: String,
        authorUsername: String,
        longFormatter: LongPostDateFormatter,
        numericFormatter: NumericPostDateFormatter
    ) throws -> ViewBlogPostWithoutTags {
        ...
        let created = created
        return try ViewBlogPostWithoutTags(
            id: self.id,
            title: self.title,
            ...
            createdDateLong: longFormatter.formatter.string(from: created),
            createdDateNumeric: numericFormatter.formatter.string(from: created),
            lastEditedDateNumeric: lastEditedNumeric,
    ...
    func toViewPost(
        ...
        let viewPost = try self.toViewPostWithoutTags(
            authorName: authorName,
            authorUsername: authorUsername,
        ...
        return ViewBlogPost(
            ...
            contents: viewPost.contents,
            author: viewPost.author,
            created: viewPost.created,
            lastEdited: viewPost.lastEdited,
            slugUrl: viewPost.slugUrl,
            published: viewPost.published,
            longSnippet: viewPost.longSnippet,
            createdDateLong: viewPost.createdDateLong,
            createdDateNumeric: viewPost.createdDateNumeric,
            lastEditedDateNumeric: viewPost.lastEditedDateNumeric,
...
extension Array where Element: BlogPost {
    func convertToViewBlogPosts(
        authors: [BlogUser.Public],
        tagsForPosts: [UUID: [BlogTag]],
        longDateFormatter: LongPostDateFormatter,
        numericDateFormatter: NumericPostDateFormatter
    ) throws -> [ViewBlogPost] {
        let viewPosts = try self.map { post -> ViewBlogPost in
            ...
            return try post.toViewPost(
                authorName: authors.getAuthorName(id: authorID),
                authorUsername: authors.getAuthorUsername(id: authorID),
                longFormatter: longDateFormatter,
                numericFormatter: numericDateFormatter,
                tags: tagsForPosts[blogID] ?? []
            )
        }
        return viewPosts
    }
    
    func convertToViewBlogPostsWithoutTags(
        authors: [BlogUser.Public],
        longDateFormatter: LongPostDateFormatter,
        numericDateFormatter: NumericPostDateFormatter
    ) throws -> [ViewBlogPostWithoutTags] {
        let viewPosts = try self.map { post -> ViewBlogPostWithoutTags in
            let authorID = post.$author.id
            return try post.toViewPostWithoutTags(
                authorName: authors.getAuthorName(id: authorID),
                authorUsername: authors.getAuthorUsername(id: authorID),
                longFormatter: longDateFormatter,
                numericFormatter: numericDateFormatter
